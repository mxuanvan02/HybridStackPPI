...
# Globals for fork worker
GLOBAL_MOTIFS = None
GLOBAL_H5 = "cache/esm2/esm2_embeddings_v4.h5"

def extract_single(args):
    seq_id, sequence = args
    from hybridstack.feature_engine import InterpretableFeatureExtractor
...
def main():
    ...
    # Init computer (won't download due to offline mode)
    ...
    for dataset in ["human", "yeast"]:
        ...
        print(f"Extracting features for {len(needed_seqs)} sequences...")
        tasks = [(seq_id, seq) for seq_id, seq in needed_seqs.items()]
        
        protein_features = {}
        with ProcessPoolExecutor(max_workers=10) as executor:
            for seq_id, combined_vector in tqdm(executor.map(extract_single, tasks), total=len(tasks)):
        ...
        with h5py.File(cache_name, "w") as hf:
            ...
            hf.attrs["feature_names"] = json.dumps(list(X_df.columns)) if 'json' in sys.modules else str(list(X_df.columns))
            # Just store as raw strings
