...
class MoranAutocorrelation:
    ...
    def compute(self, sequence: str) -> np.ndarray:
        ...
        for prop_name in self.properties:
            ...
            if seq_len < 2:
                all_features.extend([0.0] * self.max_lag); continue
...
def load_data_local(fasta_path: str, pairs_path: str):
    ...
    with open(fasta_path, "r") as f:
        ...
        for line in f:
            line = line.strip()
            if line.startswith(">"): header = line.split()[0][1:]; sequences[header] = ""
...
def main():
    ...
    comp = EmbeddingComputer(model_name="facebook/esm2_t33_650M_UR50D")
    engine = FeatureEngine(h5_cache_path=GLOBAL_H5, embedding_computer=comp)
    ...
    for dataset in ["human", "yeast"]:
        ...
        with h5py.File(GLOBAL_H5, "r") as h5f:
            for sid, sequence in tqdm(needed.items()):
                h_feats = handcraft_ext.extract(sequence)
                seq_upper = sequence.upper()
                m_key, g_key = f"{seq_upper}_matrix_v2", f"{seq_upper}_global_v2"
                if m_key in h5f and g_key in h5f:
                    emb_matrix, g_emb = h5f[m_key][:], h5f[g_key][:]
                else:
                    emb_matrix = np.zeros((len(sequence), 1280), dtype=np.float32)
                    g_emb = np.zeros(1280, dtype=np.float32)
                
                motif_vec, local_embs = [], []
                if emb_matrix.shape[0] == 0:
                    motif_vec = [0.0] * len(motifs_dict)
                    l_emb = np.zeros(2560, dtype=np.float32)
                else:
                    for pattern in motifs_dict.values():
                    ...
                    if local_embs:
                        ...
                        l_emb = np.concatenate([np.max(stacked, axis=0), np.mean(stacked, axis=0)])
                    else:
                ...
                protein_features[sid] = np.concatenate([h_feats, np.array(motif_vec, dtype=np.float32), g_emb, l_emb])
