...
class AACExtractor:
    ...
    def compute(self, sequence: str) -> np.ndarray:
        seq = sequence.upper(); length = len(seq) or 1; counts = Counter(seq)
...
class CTDExtractor:
    def __init__(self):
        ...
        for prop in CTD_GROUPS.keys():
            ...
            for i in range(3): self.feature_names.extend([f"CTD_{prop}_D_{i+1}_{q}" for q in [0, 25, 50, 75, 100]])
    def compute(self, sequence: str) -> np.ndarray:
        seq = sequence.upper(); seq_len = len(seq) or 1; features = []
        for prop, mapper in self.prop_mappers.items():
            seq_groups = [mapper.get(aa, -1) for aa in seq]
            counts = Counter(seq_groups); features.extend([counts.get(i, 0) / seq_len for i in range(3)])
            ...
            for gid in range(3):
                ...
                else: features.extend([0.0] * 5)
        return np.array(features, dtype=np.float32)

class PAACExtractor:
    def __init__(self, lambda_val: int = 10, weight: float = 0.05):
        self.lambda_val = lambda_val; self.weight = weight; self.aac_extractor = AACExtractor()
        self.feature_names = [f"PAAC_AAC_{aa}" for aa in AMINO_ACIDS] + [f"PAAC_lambda_{i+1}" for i in range(lambda_val)]
    def compute(self, sequence: str) -> np.ndarray:
        seq = sequence.upper(); seq_len = len(seq) or 1; aac = self.aac_extractor.compute(seq)
        hydro = [PHYSICO_INDICES["hydrophobicity"].get(aa, 0) for aa in seq]; hydrophil = [PHYSICO_INDICES["hydrophilicity"].get(aa, 0) for aa in seq]
        theta = []
        ...
        denom = 1 + self.weight * sum(theta)
        return np.concatenate([aac / denom, (self.weight * np.array(theta)) / denom]).astype(np.float32)

class MoranAutocorrelation:
    def __init__(self, max_lag: int = 30):
        self.max_lag = max_lag; self.properties = list(PHYSICO_INDICES.keys())
        self.feature_names = [f"Moran_{p}_lag{l}" for p in self.properties for l in range(1, max_lag + 1)]
    def compute(self, sequence: str) -> np.ndarray:
        seq = sequence.upper(); seq_len = len(seq); all_features = []
        for prop_name in self.properties:
            prop_seq = np.array([PHYSICO_INDICES[prop_name].get(aa, 0) for aa in seq], dtype=np.float32)
            if seq_len < 2: all_features.extend([0.0] * self.max_lag); continue
            mean_prop = np.mean(prop_seq); std_dev = np.std(prop_seq) + 1e-9; norm_prop_seq = (prop_seq - mean_prop) / std_dev
            for lag in range(1, self.max_lag + 1):
                if seq_len > lag: all_features.append(np.sum(norm_prop_seq[:-lag] * norm_prop_seq[lag:]) / (seq_len - lag))
...
def init_worker():
    ...
    # Load motifs in worker to avoid pickling issues
    ELM_CACHE_PATH = "data/elm_motifs_freeze.tsv"
    df = pd.read_csv(ELM_CACHE_PATH, sep="\t", comment="#")
    df = df.dropna(subset=["Regex"])
    GLOBAL_MOTIFS = {row["ELMIdentifier"]: re.compile(row["Regex"]) for _, row in df.iterrows()}

def extract_single(args):
    seq_id, sequence = args
    h_feats = InterpretableFeatureExtractor().extract(sequence)
    with h5py.File(GLOBAL_H5, "r", swmr=True) as h5f:
        seq_upper = sequence.upper(); m_key, g_key = f"{seq_upper}_matrix_v2", f"{seq_upper}_global_v2"
        if m_key in h5f and g_key in h5f: emb_matrix, g_emb = h5f[m_key][:], h5f[g_key][:]
        else: emb_matrix = np.zeros((len(sequence), 1280), dtype=np.float32); g_emb = np.zeros(1280, dtype=np.float32)
    motif_vec, local_embs = [], []
    if emb_matrix.shape[0] == 0:
        motif_vec = [0.0] * len(GLOBAL_MOTIFS); l_emb = np.zeros(2560, dtype=np.float32)
    else:
        for pattern in GLOBAL_MOTIFS.values():
            ...
            if matches:
                ...
                for m in matches:
                    s, e = m.span()
                    if s < emb_matrix.shape[0]: local_embs.append(emb_matrix[s:min(e, emb_matrix.shape[0])])
            else: motif_vec.append(0.0)
        if local_embs:
            stacked = np.vstack(local_embs); l_emb = np.concatenate([np.max(stacked, axis=0), np.mean(stacked, axis=0)])
        else:
            l = emb_matrix.shape[0]; reg = emb_matrix[l//4 : l-l//4] if l//4 < l-l//4 else emb_matrix
            l_emb = np.concatenate([np.max(reg, axis=0), np.mean(reg, axis=0)])
    return seq_id, np.concatenate([h_feats, np.array(motif_vec, dtype=np.float32), g_emb, l_emb])
...
def main():
    ...
    for dataset in ["human", "yeast"]:
        ...
        print(f"📊 {dataset.upper()}: {len(needed)} unique proteins from {len(pairs_df)} pairs.", flush=True)
        
        tasks = [(sid, s) for sid, s in needed.items()]
        protein_features = {}
        
        # Check for checkpoint
        ckpt_path = Path("cache") / f"{dataset}_protein_features_ckpt.pkl"
        if ckpt_path.exists():
            print(f"🔄 Loading protein features from checkpoint {ckpt_path}...", flush=True)
            with open(ckpt_path, "rb") as f:
                protein_features = pickle.load(f)
        
        missing_tasks = [(sid, s) for sid, s in tasks if sid not in protein_features]
        
        if missing_tasks:
            print(f"🧠 Extracting for {len(missing_tasks)} missing sequences...", flush=True)
            with ctx.Pool(processes=8, initializer=init_worker) as pool:
                for sid, vec in tqdm(pool.imap_unordered(extract_single, missing_tasks), total=len(missing_tasks), desc=f"Extracting {dataset}"):
            ...
            print(f"✅ All {len(tasks)} proteins already extracted.", flush=True)
        
        print("🏗️ Creating feature matrix (Memory-Efficient)...", flush=True)
        from hybridstack.feature_engine import FeatureEngine
        # We only use FeatureEngine for names here, so use a mock to avoid torch imports
        mock_comp = MockEmbeddingComputer(embedding_dim=1280)
        engine = FeatureEngine(h5_cache_path=GLOBAL_H5, embedding_computer=mock_comp)
        feature_names = engine.get_feature_names()
        
        # --- Memory-Efficient Matrix Creation ---
        num_pairs = len(pairs_df)
        num_individual_features = len(feature_names)
        
        # Get pairing math
        strategy = "hadamard_abs"
        num_pair_features = num_individual_features * 2 if strategy in ["hadamard_abs", "symmetric", "avgdiff"] else num_individual_features
        
        print(f"📊 Allocating matrix: {num_pairs} pairs x {num_pair_features} features", flush=True)
        X_np = np.zeros((num_pairs, num_pair_features), dtype=np.float32)
        y_np = np.zeros(num_pairs, dtype=np.float32)
        
        pair_feature_names = []
        if strategy == "hadamard_abs":
            pair_feature_names = [f"hadamard_{n}" for n in feature_names] + [f"absdiff_{n}" for n in feature_names]
        else:
            pair_feature_names = feature_names
        
        for i, (idx, row) in enumerate(tqdm(pairs_df.iterrows(), total=num_pairs, desc=f"Building {dataset} matrix")):
            p1, p2, label = row["protein1"], row["protein2"], row["label"]
            if p1 > p2: p1, p2 = p2, p1
            
            try:
                f1, f2 = protein_features[p1], protein_features[p2]
                if strategy == "hadamard_abs":
                    combined = np.concatenate([f1 * f2, np.abs(f1 - f2)])
        ...
        X_df = pd.DataFrame(X_np, columns=pair_feature_names, index=pairs_df.index)
        y_s = pd.Series(y_np, index=pairs_df.index, name="label")
        # ----------------------------------------
        
        c_name = f"cache/{dataset}_{dataset}_pairs_same_go_facebook_esm2_t33_650m_ur50d_hadamard_abs_v3_features.h5"
        print(f"💾 Saving to {c_name}...", flush=True)
