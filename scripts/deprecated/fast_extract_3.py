...
def init_worker():
    ...
    # Initialize a dummy engine just to load motifs safely in worker
    engine = FeatureEngine(h5_cache_path=GLOBAL_H5)
    GLOBAL_MOTIFS = engine.motifs
...
def main():
    for dataset in ["human", "yeast"]:
        ...
        protein_features = {}
        # Using fork here is safe because PyTorch has NOT been imported in the main thread yet!
        ...
        from hybridstack.feature_engine import FeatureEngine
        feature_engine = FeatureEngine(h5_cache_path=GLOBAL_H5)
